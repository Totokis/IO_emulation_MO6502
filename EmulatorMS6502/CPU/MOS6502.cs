 ...
 U = (1 << 5),   // Unused               00100000
        ...
        */
        ...
        Byte ReadFromBus(UInt16 address) {
            Console.WriteLine("Read from bus address: "+ address);
        ...
        {
            //LDA
            Console.WriteLine("Start Demo RAM:");
            this.bus.ram.ToList().ForEach(i => Console.Write(i+" "));
            opcode = ReadFromBus(programCounter);
            programCounter++;
            lookup[opcode].AdressingMode();
            ...
            lookup[opcode].AdressingMode();
            lookup[opcode].Opcode();
            Console.WriteLine("End Demo RAM:");
            this.bus.ram.ToList().ForEach(i => Console.Write(i+" "));
        }
    }
}
